// /api/vendors/[id] — the full vendor profile, and edits to it.
import { updateVendorSchema } from "@/lib/schemas/procurement";
...
export const PATCH = withUser<{ id: string }>(async ({ req, params, principal, context }) =>
  service.update({ principal, context }, params.id, await parseBody(req, updateVendorSchema))
);
